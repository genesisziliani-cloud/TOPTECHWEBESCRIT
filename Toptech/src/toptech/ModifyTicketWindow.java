 ...
 * cuando managers tienen firmas distintas (updateTicket devuelve void o
 * boolean, PaymentManager puede no tener deletePaymentByTicketId,
 * ListUsersWindow puede no exponer refresh()).
 *
 * Cambios principales: - Llama a ticketManager.updateTicket(...) por reflexión
 * y acepta tanto void como boolean. - Llama a userManager.findUserByDNI(...) y
 * userManager.updateUser(...) por reflexión (si existen). - Llama a
 * paymentManager.deletePaymentByTicketId(...) por reflexión (si existe). - Para
 * refrescar listas usa reflexión: intenta invocar "refresh" en cada ventana
 * abierta; evita referencias directas a ListUsersWindow.refresh() que provocan
 * errores de compilación cuando la firma no existe.
 *
 * Reemplaza src/toptech/ModifyTicketWindow.java por este archivo (haz backup
 * antes).
 */
public class ModifyTicketWindow extends javax.swing.JFrame {
    ...
    public ModifyTicketWindow(TicketManager ticketManager, UserManager userManager, JFrame parentWindow, String rol) {
        if (ticketManager == null) {
            throw new IllegalArgumentException("ticketManager cannot be null");
        }
    ...
    private void initComponents() {
        ...
        txtIdTicket.setBounds(100, 48, 140, 22);
        p.add(txtIdTicket);

        btnBuscar = new JButton("BUSCAR");
        btnBuscar.setBounds(250, 48, 100, 22);
        btnBuscar.addActionListener(e -> publicOnBuscar());
        p.add(btnBuscar);

        JLabel lblDni = new JLabel("D.N.I / RUC");
        lblDni.setBounds(380, 48, 100, 20);
        p.add(lblDni);
        txtDni = new JTextField();
        txtDni.setBounds(480, 48, 140, 22);
        p.add(txtDni);

        JLabel lblCliente = new JLabel("CLIENTE");
        lblCliente.setBounds(20, 88, 70, 20);
        p.add(lblCliente);
        txtCliente = new JTextField();
        txtCliente.setBounds(100, 88, 300, 22);
        p.add(txtCliente);

        JLabel lblEquipo = new JLabel("EQUIPO");
        lblEquipo.setBounds(420, 88, 60, 20);
        p.add(lblEquipo);
        equipo = new JComboBox<>(new String[]{"OTROS", "PC", "LAPTOP", "IMPRESORA"});
        equipo.setBounds(480, 88, 140, 22);
        p.add(equipo);

        JLabel lblEstado = new JLabel("ESTADO");
        lblEstado.setBounds(640, 88, 60, 20);
        p.add(lblEstado);
        estado = new JComboBox<>(new String[]{"ASIGNADO", "ATENCION", "SOLUCIONADO", "CANCELADO"});
        estado.setBounds(700, 88, 160, 22);
        p.add(estado);

        JLabel lblDesc = new JLabel("DESCRIPCION");
        lblDesc.setBounds(20, 128, 100, 20);
        p.add(lblDesc);
        txtDescripcion = new JTextArea();
        JScrollPane spDesc = new JScrollPane(txtDescripcion);
        spDesc.setBounds(20, 150, 560, 90);
        p.add(spDesc);

        JLabel lblFechaCre = new JLabel("FECHA DE CREACION");
        lblFechaCre.setBounds(600, 128, 140, 20);
        p.add(lblFechaCre);
        txtFechaCreacion = new JTextField();
        txtFechaCreacion.setBounds(600, 150, 260, 22);
        p.add(txtFechaCreacion);

        JLabel lblFechaFin = new JLabel("FECHA FIN");
        lblFechaFin.setBounds(600, 185, 80, 20);
        p.add(lblFechaFin);
        txtFechaFin = new JTextField();
        txtFechaFin.setBounds(600, 205, 260, 22);
        p.add(txtFechaFin);

        JLabel lblCorreo = new JLabel("CORREO");
        lblCorreo.setBounds(600, 238, 60, 20);
        p.add(lblCorreo);
        txtCorreo = new JTextField();
        txtCorreo.setBounds(660, 238, 200, 22);
        p.add(txtCorreo);

        JLabel lblCel = new JLabel("CELULAR");
        lblCel.setBounds(600, 271, 60, 20);
        p.add(lblCel);
        txtCelular = new JTextField();
        txtCelular.setBounds(660, 271, 200, 22);
        p.add(txtCelular);

        chkDiagnosticoPagado = new JCheckBox("Diagnóstico pagado");
        chkDiagnosticoPagado.setBounds(20, 260, 220, 22);
        p.add(chkDiagnosticoPagado);

        JLabel lblMonto = new JLabel("MONTO REPARACION:");
        lblMonto.setBounds(20, 320, 150, 20);
        p.add(lblMonto);
        txtMontoReparacion = new JTextField();
        txtMontoReparacion.setBounds(180, 320, 120, 22);
        p.add(txtMontoReparacion);

        chkPagado = new JCheckBox("Pagado");
        chkPagado.setBounds(310, 320, 80, 22);
        chkPagado.addActionListener(e -> onPagadoToggled());
        p.add(chkPagado);

        btnVerCapturas = new JButton("Ver Capturas");
        btnVerCapturas.setBounds(400, 320, 140, 22);
        btnVerCapturas.addActionListener(e -> onVerCapturas());
        p.add(btnVerCapturas);

        JLabel lblTecn = new JLabel("# TECNICO");
        lblTecn.setBounds(600, 10, 80, 20);
        p.add(lblTecn);
        tecnicos = new JComboBox<>();
        tecnicos.setBounds(690, 10, 170, 22);
        p.add(tecnicos);

        JLabel lblPrior = new JLabel("PRIORIDAD");
        lblPrior.setBounds(600, 40, 80, 20);
        p.add(lblPrior);
        prioridad = new JComboBox<>(new String[]{"NORMAL", "***ALTA***"});
        prioridad.setBounds(690, 40, 170, 22);
        p.add(prioridad);

        btnGuardar = new JButton("GUARDAR");
        btnGuardar.setBounds(300, 420, 130, 36);
        btnGuardar.addActionListener(e -> onGuardar());
        p.add(btnGuardar);
        btnCancelar = new JButton("CANCELAR");
        btnCancelar.setBounds(460, 420, 130, 36);
    ...
    private void fillFormFromTicket(Ticket ticket) {
        ...
        if (tecnicoActual != null && !tecnicoActual.trim().isEmpty()) {
            ...
            for (int i = 0; i < tecnicos.getItemCount(); i++) {
                if (tecnicoActual.equals(tecnicos.getItemAt(i))) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                tecnicos.addItem(tecnicoActual);
            }
    ...
    private void onPagadoToggled() {
        ...
        try {
            Method getPago = null;
            try {
                getPago = paymentManager.getClass().getMethod("obtenerPagoPorTicket", String.class);
            } catch (NoSuchMethodException ignore) {
            }
            Object existing = null;
            if (getPago != null) {
                existing = getPago.invoke(paymentManager, ticket.getId());
            }
            ...
            if (selected) {
                if (existing == null) {
                    ...
                    try {
                        ...
                        if (!ok) {
                            JOptionPane.showMessageDialog(this, "No se registró pago (error o duplicado).");
                            chkPagado.setSelected(false);
                            return;
                        }
                        refreshParentLists();
                ...
                try {
                    ...
                    Method del = null;
                    try {
                        del = paymentManager.getClass().getMethod("deletePaymentByTicketId", int.class);
                    } catch (NoSuchMethodException ignore) {
                    }
                    if (del != null) {
    ...
    private void onGuardar() {
        ...
        if (finEstado) {
            ticketOriginal.setFechaFin(dfDateTime.format(new Date()));
        } else {
            ticketOriginal.setFechaFin("");
        }
    ...
    private void onVerCapturas() {
        ...
        try {
            ...
            if (pagoObj == null) {
                JOptionPane.showMessageDialog(this, "No hay pago registrado.");
                return;
            }
